...
var GeoSetup = (function() {
	...
	var store = {
		...
		"320500" : [13414558, 3664736, 13440031, 3676201],// 苏州大市
	...
	return {
		...
		setSR : function(cordSystem) {
			...
			if (cordSystem !== pgis && cordSystem !== gmap) {
				alert("不存在坐标类型，请检查是否输入错误");
				return false;
			}

			corSystem = cordSystem;
		 ...
		 *            cordSystem 坐标系 已存在的地区及其名称简写 无锡--"320200" 南通通州--"320683"
		 *            海南文昌--"469005" 苏州--"320500" 海南白沙--"BS"  镇江--"321100"
		 *            扬州--"321000Z" 宜兴--"320282"
		 */
		setLocation : function(locatn, cordSystem) {
			if (!checkArgs(arguments)) {
				return;
			}
			location = locatn;
			this.setSR(cordSystem);
			return this;
		},
		/**
		 * 获得范围
        ...
        /**
         * 根据地理坐标获得两点距离
         * @param MapPoint1 地理坐标点1
         * @param MapPoint2 地理坐标点2
         * @return s 距离
         */
        getDistance:function(MapPoint1,MapPoint2){
            var radLat1=rad(MapPoint1.getY());
            var radLat2=rad(MapPoint2.getY());
            var a=radLat1-radLat2;
            var b=rad(MapPoint1.getX())-rad(MapPoint2.getX());
